...
export const addNote = (planId, title, description, spent_time) => {
  return (dispatch, getState) => {
    let headers = {"Content-Type": "application/json"};
    let {token} = getState().auth;
    ...
    let body = JSON.stringify({planId, title, description, spent_time, });
...
export const updateNote = (index, planId, title, description, spent_time) => {
  return (dispatch, getState) => {
